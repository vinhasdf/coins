...
class coin:
    def __init__(self, name):
        url = "https://api.binance.com/api/v3/klines?interval=1d&limit=270&symbol=" + name
        r = requests.get(url)
        self.coin_prices = []
        # 15 array
        ...
        if (len(info) >= 15):
            for i in range(0, len(info)):
                self.coin_prices.append(float(info[i][4]))
...
#     testlist.getlist()[-1] = target_price(testlist.getlist(), 70)
...
# store all coin to dictionary and sort by rsi
if (len(testlist.getlist()) > 0):
    testlist.getlist()[-1] = target_price(testlist.getlist(), 80)
    tempstr = (str(nline) + '\t' + str(calrsi14_ver2(testlist.getlist())) +
    '\t' + str(len(testlist.getlist())) + ' ' + str(testlist.getlist()[-1]) + '\n')
    tempstr = tempstr.replace('.', ',')
    print(tempstr)